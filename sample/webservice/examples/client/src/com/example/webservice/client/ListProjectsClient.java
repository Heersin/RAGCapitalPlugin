...
/**
 * A concrete implementation for the CIS "ListProjects" web service client.
 */
public class ListProjectsClient extends AbstractClient
{
	public ListProjectsClient() throws Exception
	{
	}
	...
	protected void processResponse(Document responsePayload) throws Exception
	{
		...
		NodeList nodes = responsePayload.getElementsByTagName("project");
		int nbNodes = nodes.getLength();

		logMsg("");
		logMsg("Projects: " + nbNodes);
		for (int i = 0; i < nbNodes; i++) {
			Element project = (Element) nodes.item(i);
			logMsg("> Project name: " + project.getAttribute("name"));
			logMsg("          description: " + project.getAttribute("description"));
			logMsg("          id: " + project.getAttribute("id"));
		}
	}
}
