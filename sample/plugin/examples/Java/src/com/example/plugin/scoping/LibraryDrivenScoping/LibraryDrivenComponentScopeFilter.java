...
public class LibraryDrivenComponentScopeFilter implements IXComponentScopeSelectionFilter
{
		...
		if (scopingContext != null) {
			...
			for(Object obj :objs)
			{
				  if (obj instanceof IXLibraryObject) {
					if (isComponentApplicable((IXLibraryObject) obj, scopingContext)) {
	...
	private boolean isComponentApplicable(IXLibraryObject libObj, IXComponentSelectionContext scopingContext)
	{
		...
		for (IXLibraryComponentScope partScope : partScopes) {
			if(!LibraryDrivenScopeValidator.isPartScopeValid(partScope.getScope(), designScopes, partScope.getAttribute(IXAttributes.ScopeType)))
				return false;
		}
		return true;
	}
